...
for expi = 1:len
    ...
    for j = 1:iter
        ...
        %---------------------------------------------------%
        % learning
        [Wt,bt,Pt,Ps] = DDA(Xs,Xs_Label,Xl,Xl_Label,Xu,Xu_Label,beta,tau,lambda,d,T);        
        %---------------------------------------------------%
        % prediction
        [~,ft_preLabel_u] = Linear(Xu,Wt,bt,Pt);   % ft(xu);
        [~,ft_preLabel_l] = Linear(Xl,Wt,bt,Pt);   % ft(xl);
        [~,ft_preLabel_s] = Linear(Xs,Wt,bt,Ps);   % ft(xs);
        %---------------------------------------------------%

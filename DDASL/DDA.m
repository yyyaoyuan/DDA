function [Wt,bt,Pt,Ps] = DDA(Xs,Xs_Label,Xl,Xl_Label,Xu,Xu_Label,beta,tau,lambda,d,T)
%-----------------------------------------%
%CTF algorithm
% Input:
...
[~,pseudo_Xu_Label_ft] = Linear(Xu,Wt,bt,Pt);   % ft(xu);
...
for i = 1:T
    ...
    [~,pseudo_Xu_Label_ft] = Linear(Xu,Wt,bt,Pt);   % ft(xu);
    [~,pseudo_Xl_Label_ft] = Linear(Xl,Wt,bt,Pt);   % ft(xl);
    [~,pseudo_Xs_Label_ft] = Linear(Xs,Wt,bt,Ps);   % ft(xs);
